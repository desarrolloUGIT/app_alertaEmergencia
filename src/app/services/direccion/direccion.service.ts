...
export class DireccionService {
  ...
  dpa(){
    ...
    let sr = `<soapenv:Envelope [env]:soapenv="http://schemas.xmlsoap.org/soap/envelope/" [env]:max="http://www.ibm.com/maximo">
              <soapenv:Header/>
              <soapenv:Body>
                <max:QueryMOP_DOMAIN_DOH >
                    <max:MOP_DOMAIN_DOHQuery operandMode="AND">
                      <max:MAXDOMAIN>
                          <max:DOMAINID >=COMUNAS</max:DOMAINID>
                      </max:MAXDOMAIN>
                    </max:MOP_DOMAIN_DOHQuery>
                </max:QueryMOP_DOMAIN_DOH>
              </soapenv:Body>
          </soapenv:Envelope>`
      let url = URL_SERVICIOS+'MOP_WS_MOP_DOMAIN_DOH';
      const options: HttpOptions = {
        url:url,
        data:sr,
        headers:this.headers
